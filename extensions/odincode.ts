...
function getOdinFace(theme: Theme): string[] {
  ...
  return [
    ...
    `    ${hair(B)}${skin(B)}${brow(B+B)}${skin(B+B)}${brow(B+B)}${skin(B+B)}${hair(B)}`,
    `    ${hair(B)}${skin(B)}${eye(S+R)}${skin(B+B)}${eye(S+R)}${skin(B+B)}${hair(B)}`,
    `    ${skin(B+B+B+B)}${lip(H+H)}${skin(B+B+B+B+B)}`,
    `    ${skin(B+B+B)}${lip(B+B+B+B)}${skin(B+B+B+B)}`,
